...
BioIKSolver::BioIKSolver(const robot_state::JointModelGroup &all_joints_group, const robot_state::JointModelGroup &legs_joints_group, const robot_state::JointModelGroup &lleg_joints_group, const robot_state::JointModelGroup &rleg_joints_group){
    _use_approximate = true;
    _bioIKSingleChain = false;
    _bioIK_timeout = 0.01;
    _all_joints_group = &all_joints_group;
    _legs_joints_group = &legs_joints_group;
...
bool BioIKSolver::solve(tf::Transform& trunk_to_support_foot, tf::Transform& trunk_to_flying_foot, bool is_left_support, robot_state::RobotStatePtr &robot_state_ptr){
    ...
    bool success = true;
    {
        ...
        if(_bioIKSingleChain) {
            ik_options.goals.emplace_back(fly_goal);
            success = success && robot_state_ptr->setFromIK(_legs_joints_group,           // joints which shall be used
...
void BioIKSolver::set_single_chain(bool single_chain){
    _bioIKSingleChain = single_chain;
}
