...
bool AnalyticIKSolver::solve(tf::Transform& trunk_to_support_foot, tf::Transform& trunk_to_flying_foot, bool is_left_support, robot_state::RobotStatePtr &robot_state_ptr) {
    ...
    if(_robot_type == "Wolfgang"){
        trunk_to_LHipYaw.setValue(0.0, 0.055, 0.0);
        trunk_to_RHipYaw.setValue(0.0, -0.055, 0.0);
    }else if(_robot_type == "Minibot"){
        trunk_to_LHipYaw.setValue(0.0, 0.057775, 0.0);
        trunk_to_RHipYaw.setValue(0.0, -0.057775, 0.0);
    }else{
...
bool AnalyticIKSolver::legIK(tf::Transform& goal, std::vector<double>& positions, bool isLeftLeg){
    ...
    if(_robot_type == "Wolfgang"){
        hipYawToPitchX = 0.0231;
        hipYawToPitchY = 0.0;
        hipYawToPitchZ = -0.0438;
        hip_pitch_to_knee = 0.17;
        knee_to_ankle = 0.17;
        ankle_to_sole = -0.052;
    }else if(_robot_type=="Minibot"){
        hipYawToPitchX = 0.0;
        hipYawToPitchY = 0.0;
        hipYawToPitchZ = -0.038444;
        hip_pitch_to_knee = 0.150;
        knee_to_ankle = 0.150;
        ankle_to_sole = -0.040444;
    }else{
        ROS_ERROR("Robot type is not correctly defined, analytic IK wont work.");
    }
